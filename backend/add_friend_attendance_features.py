...
def main():
    ...
    try:
        ...
        db_module = db.query(models.Module).filter(models.Module.module_name == module_name).first()
        if not db_module:
        ...
        faculty_role = db.query(models.Role).filter(models.Role.role_name == 'faculty').first()
        ...
        features_to_add = {
            ...
            "Teacher Secure Login": {"student": [], "faculty": ["create", "read", "update", "delete"]},
            "Mark Student Attendance": {"student": [], "faculty": ["create", "read", "update", "delete"]},
            "Update/Edit Attendance Records": {"student": [], "faculty": ["create", "read", "update", "delete"]},
            "View Class Attendance Reports": {"student": [], "faculty": ["create", "read", "update", "delete"]},
            "Generate Attendance Percentage": {"student": [], "faculty": ["create", "read", "update", "delete"]},
            "Search and Filter Student Records": {"student": [], "faculty": ["create", "read", "update", "delete"]},
            "Export Attendance Reports": {"student": [], "faculty": ["create", "read", "update", "delete"]},
        }
        ...
        for f_name, role_perms in features_to_add.items():
            ...
            if not db_feature:
                db_feature = models.Feature(feature_name=f_name, module_id=db_module.module_id)
                db.add(db_feature)
                db.commit()
            ...
            if student_role:
                for action in role_perms["student"]:
                    ...
                    if not rp:
                        ...
                        rp = models.RolePermission(role_id=student_role.role_id, permission_id=perm.permission_id)
                        db.add(rp)
            ...
            if faculty_role:
                for action in role_perms["faculty"]:
                    ...
                    if not rp:
                        ...
                        rp = models.RolePermission(role_id=faculty_role.role_id, permission_id=perm.permission_id)
                        db.add(rp)
